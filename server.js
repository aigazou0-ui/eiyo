...
const ENGINE_SKIP_READY = String(process.env.YANEURAOU_SKIP_READY || "1") !== "0";
...
class UsiEngine {
  ...
  async ensureStarted() {
    ...
    if (ENGINE_SKIP_READY) {
      console.log("[engine] ready check skipped");
    } else {
      this.send("isready");
